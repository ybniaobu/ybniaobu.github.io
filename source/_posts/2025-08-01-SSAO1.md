...
对于 AO 效果而言，我们更关心几何边缘的维持，所以过滤时需要额外考虑几何信息，比如 depth 和 normal 值，我自己实测下来，只考虑 depth 差异的效果比只考虑 normal 差异的效果要好很多，因为 normal 的差异无法区分两个平行的平面但有深度差异的情形。当然 depth 和 normal 同时考虑的效果肯定更好，但是为了减少采样次数，只考虑 depth 差异的效果已经基本够用了。
...
上述代码中，depthDelta 是在世界/观察空间下的线性深度差，基本上严格参照了双边过滤的公式。我也有看到其他代码（NVIDIA）将深度测试更加简单化了，直接使用一个阈值进行测试，如下：  
...
### 优化
如果使用像素/片元着色器进行双边过滤，拆分为水平和垂直两次，那么每次每个像素都需要采样核长度的样本，假设核长度为 9，每个像素就都要采样 9 次。
