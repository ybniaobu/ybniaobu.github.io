...
# 第十六课 Ray Tracing IV
**概率分布函数 Probability Distribution Function (PDF)**：$\,X \sim p(x)\,$，它描述的是连续的变量 X 的概率 p(x) 函数。

概率 $\,p(x) \geq 0\,$，并且 p(x) 的积分为 1（所有值的概率合计为 1）：$\,\int p(x) dx = 1\,$

那么对于期望值就是所有值 X 乘上所有值的概率的合计，即 $\,E(X) = \int x p(x) dx\,$
