...
<img src="https://s2.loli.net/2025/04/28/OSvIorZFNbdnauC.png" width = "20%" height = "20%" alt="图129 - FXAA Neighborhood Samples"/>
    ...
    float4 SampleOffset(float2 uv, int uOffset, int vOffset)
    {
...
当然只做一步是不够的，我们需要循环遍历，将上面代码删除。因为不可能无限循环去搜索，我们需要假设一个最大循环步长，先假设为 10 步，我们需要向两端都进行搜索，同时若搜索步长超过了 10 步还是没有找到边界，则默认边界像素距离长度额外加 8：  
    ...
    float lumaGradientP, lumaGradientN, distanceP, distanceN;
    int i;
    float edgeDistance = isHorizontal ? edgeStep.x : edgeStep.y;
    
    UNITY_UNROLL
    for (i = 1; i <= EXTRA_EDGE_STEPS; i++)
    {
        lumaGradientP = GetLuma(SampleOffsetZero(edgeUV + i * edgeStep)) - edgeLuma;
        if (abs(lumaGradientP) > gradientThreshold)
        {
            distanceP = i * edgeDistance;
            break;
        }
    }
    ...
    {
        distanceP = edgeDistance * (EXTRA_EDGE_STEPS + LAST_EDGE_STEP_GUESS);
    }
    
    UNITY_UNROLL
    for (i = 1; i <= EXTRA_EDGE_STEPS; i++)
    {
        lumaGradientN = GetLuma(SampleOffsetZero(edgeUV - i * edgeStep)) - edgeLuma;
        if (abs(lumaGradientN) > gradientThreshold)
        {
            distanceN = i * (isHorizontal ? edgeStep.x : edgeStep.y);
            break;
        }
    }
    ...
    if (i == EXTRA_EDGE_STEPS + 1)
    {
        distanceN = edgeDistance * (EXTRA_EDGE_STEPS + LAST_EDGE_STEP_GUESS);
    }
