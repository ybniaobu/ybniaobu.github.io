...
**①**本来我是想用 Computer Shader 对 Cubemap 进行预过滤的，但中间遇到了很多问题，首先是 HLSL 不存在 RWTextureCube，虽然可以对 TextureCube 进行采样。所以我们只能使用 RWTexture2DArray。但是因为我们要根据不同的粗糙度，存储到不同的 Mipmap 中，假设存储 9 个粗糙度，就需要让 RWTexture2DArray 有 6 个面（dimension）以及 9 个 mipmap。在 Computer Shader 里预过滤后，还需将 RWTexture2DArray 的每个 mipmap 的每个面的 GPU 数据传递到 Cubemap 的每个 mipmap 的每个面的 CPU 数据中。但是 GPU 数据传递给 CPU 就比较麻烦了，Texture2D 有 ReadPixels API 可以将 GPU 数据拷贝到 CPU 中，但是 Texture2DArray、TextureCube 却没有类似的 API。我最后选择用 RWStructuredBuffer 传递预过滤数据，最后实现了有个问题，就是高粗糙下的预过滤 Cubemap 存在边缘接缝的衔接问题，我不知道如何解决。
...
**⑤**因为之后肯定是要用到全局光照技术的，我也不知道这里遇到的问题有没有必要去解决，所以我打算之后真的遇到了有了这个需求再处理。
