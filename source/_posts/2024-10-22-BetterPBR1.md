...
我的解决方案是修改 energyCompensation 项的变化率，让其更适配 Renormalized Disney diffuse 模型，试了试，发现开根号或者直接除以 2 都还行：  
    ...
    float3 energyCompensation = 1.0 + sqrt(F0 * (1.0 / envBRDF.y - 1.0));
    float3 energyCompensation = 1.0 + F0 * (1.0 / envBRDF.y - 1.0) / 2;
...
### Prefilter Environment Map 的实践问题
在预过滤 Cubemap 的时候遇到了很多实践上的问题，我觉得有必要阐述一遍，说实话这里浪费了我不少时间。

**①**本来我是想用 Computer Shader 对 Cubemap 进行预过滤的，但是在 HLSL 不存在 RWTextureCube，虽然可以对 TextureCube 进行采样。所以我们只能使用 RWTexture2DArray。但是因为我们要根据不同的粗糙度，存储到不同的 Mipmap 中，假设存储 6 个粗糙度，就需要 6 个 RWTexture2DArray，每个 Texture2DArray 需要有六个面。将 Texture2DArray 传递回 CPU 后，还得在 CPU 里将每个 Texture2DArray 的每个面复制给 Cubemap 的每个 mipmap 的每个面。说实话不是不能写，但真的太繁琐了，所以我没这么做。

**②**我后来就想到了 Unity 的 Camera.RenderToCubemap API，并且直接在 Skybox 的 Shader 里进行预过滤，我调用了多次 Camera.RenderToCubemap 分别将不同粗糙度的预过滤 Skybox 渲染到了多个不同分辨率的 Cubemap 里（因为 Camera.RenderToCubemap 无法直接渲染到 mipmap 里），最后再将不同分辨率的 Cubemap 复制到一个 Cubemap 的不同 mipmap 里。最后用 AssetDatabase.CreateAsset 创建 Cubemap 资产，在创建前还可以用 EditorUtility.CompressCubemapTexture 对 Cubemap 进行压缩。

TODO：RenderToCubemap API 会将进行 gamma 0.45 处理看起来会更白

**③**Unity 自带的 specular 卷积是 Phong

TODO2：补充 roughness 到 mipmap level 的映射函数，而不是现在的简单线性映射
