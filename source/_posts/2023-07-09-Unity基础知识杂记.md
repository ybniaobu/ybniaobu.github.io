...
> 本文章仅记录 unity 的基础知识，内容拓展详见其他文章。  
> 本文章参考达内教育祁天暄的 unity 教程、《Unity 3D 游戏开发》以及 Unity 官方文档。
...
## 协程 coroutine
Unity 提供了一套协程的机制（不是多线程，协程在主线程上执行）。Unity 自己本身 UnityEngine API （即 Unity 脚本）是不能被多线程调用的，但是仍可使用多线程去执行其他需要大量计算的内容。
...
序列化是 Unity 保存游戏数据的一个自动化进程。脚本中声明为 public 的对象都支持序列化格式。若要同时支持序列化格式以及私有化这些数据不让外部可以直接访问，可以使用特性：`[SerializeField]`，并使用 private 声明对象。
...
① Render Mode  
Screen Space - Overlay 覆盖模式：让 UI 元素绘制在其他元素之前，且绘制过程独立于场景元素和摄像机设置，画布尺寸由屏幕大小和分辨率决定；  
&emsp;&emsp; · Pixel Perfect 完美像素：开启可以锐化屏幕显示效果；  
&emsp;&emsp; · Sort Order 渲染顺序：在多个 Canvas 中，值越大越渲染到最上层；  
Screen Space - Camera：选择一个相机后，按照和相机的距离前后显示物体和 UI。相机移动或参数改变，画布也会跟着变化；  
&emsp;&emsp; · Plane Distance：画布与摄像机的距离；  
&emsp;&emsp; · Sorting Layer：通过 Tags & Layers 调整画布的渲染顺序；  
World Space：画布渲染于世界空间，与场景中其他 3D 物体一样。
