...
④在 Hierarchy 窗口中右键选择 2D Project --> Tilemap 创建，此时 Scene 窗口中将出现网格，用选取器选中调色板中的图片，直接在 Scene 窗口中单击创建即可。可以创建多个 Tilemap，将其层级排序；  
⑤根据需求添加 Tilemap Collider 2D 组件。

### Tilemap 相关参数
①瓦片的网格布局包括 Rectangle、Hexagon、Isometric 等距瓦片、Isometric Z as Y。
