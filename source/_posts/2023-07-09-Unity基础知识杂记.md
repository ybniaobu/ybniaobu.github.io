...
# 三维数学基础
这里只记录简单的数学知识以及代码操作，复杂的详见矩阵等相关数学文章。
## 向量基础
一、向量相关API  
...
Unity API：`Vector3.Dot(a, b)`

三、向量**叉乘 cross product**  
又称 *叉积* 、 *外积* 或者 *向量积* ，叉乘结果为**向量 vector**，并且结果向量（向量 c）与向量 a 和向量 b 组成的平面垂直，方向遵守右手定则（四指从向量 a 旋转到向量 b，那么大拇指为向量 c 的方向）。若 $\vec a = ( a_1, a_2, a_3 )$，$\vec b = ( b_1, b_2, b_3 )$，i、j、k 分别为X、Y、Z 轴方向的单位向量，公式为：  
...
向量 c 的模长也就是向量 a 和 向量 b 构成的平行四边形的面积
