...
又称 *叉积* 、 *外积* 或者 *向量积* ，叉乘结果为**向量 vector**，并且结果向量（向量 c）与向量 a 和向量 b 组成的平面垂直，即**法向量**，方向遵守右手定则（四指从向量 a 旋转到向量 b，那么大拇指为向量 c 的方向）。若 $\vec a = ( a_1, a_2, a_3 )$，$\vec b = ( b_1, b_2, b_3 )$，i、j、k 分别为X、Y、Z 轴方向的单位向量，公式为：  
