...
### 碰撞器组件属性
①**Is Trigger**：碰撞体会默认阻挡刚体的运动，但是有些时候需要检测两个物体发生重叠但又不想引起物理上的碰撞，就需要勾选此选项，将碰撞体变成一个触发器。当勾选此项后，该物体就不会再阻挡刚体运动了，但会触发下面方法：OnTriggerEnter、OnTriggerExit、OnTriggerStay  
