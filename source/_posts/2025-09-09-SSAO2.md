...
Bent Normal 可以用一张 RT 在计算 GTAO 时顺带输出，但问题在于 Bent Normal 也是有噪点的，也需要降噪，并且对方向直接进行混合平均是有一定误差的，所以过滤的效果不是那么好。
...
cone/cone intersection; cone/lobe intersection（3d look up table）; 4d look up table

---

GTSO 使用了类似于 spilt integral approximation 的方法，对渲染方程的 visibility 项也进行了拆分
...
假设 BRDF 和 visibility 都可以用圆锥体 cone 近似表达，那么 specular occlusion 可以通过这两个 cone 的交集来表达，如下图：  

【图】

Bent Normal 决定 Visibility Cone Direction，AO 决定 Visibility Aperture $\,\alpha_v\,$；Reflection Direction 决定 Reflection Cone Direction，Roughness 决定 Reflection Aperture $\,\alpha_s\,$。
...
**②**Reflection Aperture $\,\alpha_s\,$ 的计算则比较麻烦，因为理论上来说 Reflection Lobe 不是一个圆锥体，所以没有绝对的方案，PPT 中提供的方案是基于 Phong 波瓣的，使用的参数是光滑度 $\,p\,$ 如下：  

$$ \alpha_s = acos\left(0.01^{\cfrac{1}{P + 2}}\right) $$
