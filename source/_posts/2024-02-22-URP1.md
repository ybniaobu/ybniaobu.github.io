...
mathjax: true
description: 主要内容有 Unity SRP 介绍、URP 相关概念、HLSL 基础语法、URP shader 代码大致框架、URP 透明度测试和透明度混合以及 GPU Instancing。
---

> 之前的《Unity Shader入门精要》是以 Unity 内置渲染管线为基础的，以 Cg 语言为主，但当使用 URP 和 HDRP 时，使用的是 HLSL 语言；  
> 本文简单介绍 URP 相关概念，主要关注 URP 的 Shader 代码，并将部分重要的 Unity Shader 入门精要的 Cg 语言的 Shader 转换为 HLSL，以实现 NPR 渲染为结尾。
...
> 自定义渲染管线资产不再这里说明了，日后有需求详见官方文档和 https://catlikecoding.com/unity/tutorials/custom-srp/custom-render-pipeline/
...
# URP 的 HLSL 语法
> 这里主要记录 URP 中常用的，对 URP 的 Shader 有影响的语法，毕竟 URP 的 HLSL 和 HLSL 还是有区别的。想了解更深可以查阅微软的文档：https://learn.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl。

HLSL 语法是基于 C 语言的语法，和 C 很像。if 条件语句、 switch 条件语句、for 循环语句和 while 循环语句与 C 语言一致，return、continue 和 break 循环控制与 C 语言也一致。另外引入了 discard，该关键字只能在片元着色器（像素着色器）中使用，表示放弃当前像素。

## 变量 Variables
在 HLSL 中，有几种不同的变量类型，其中最常见的是标量、向量和矩阵。还有一些特殊类型：纹理 Texture 类型、采样器 Sampler 类型。数组和缓冲区也存在用于将更多的数据传递到着色器中。

### 标量 Scalar
在 URP 中主要使用下面 3 种数值类型：  
①**half** ：16-bit floating point value；  
②**float** ：32-bit floating point value；  
③**real** ：在 URP/HDRP 中使用，当函数支持 half 或 float 时。默认为 half（假定它们在平台上受支持），除非着色器指定 #define PREFER_HALF 0，否则它将使用 float 精度。ShaderLibrary 中的许多常用数学函数使用此类型；  
④fixed：HLSL 不支持。
