...
## CBuffer
**常量缓冲区 Constant Buffer** 是 GPU 显存中的一个区域，其数据可供着色器程序引用，专门用于存储一些需要 CPU 每帧更新一次的数据，比如变换矩阵、材质属性、光照信息等。
...
这样可以指示 Unity 编辑器使用 StandardForwardShaderGUI 类实例来绘制使用了 Standard Forward 着色器的材质的 inspector。这个类我创建在了 YPipeline / Editor / ShaderGUI 文件夹里，该类的基础代码如下，需要继承 `ShaderGUI` 类，并覆写 `OnGUI()` 方法，并传递进 `MaterialEditor` 和 `MaterialProperty` 数组：  
...
{
    public class StandardForwardShaderGUI : ShaderGUI 
    {
