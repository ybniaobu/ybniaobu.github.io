...
首先 ACES 流程有如下几个组成部分：  
**①Input Device Transform (IDT)**：这一步骤也称为 Input Transform，主要目的是将颜色数据转换至 ACES 色彩空间。ACES 色彩空间又包括：  
...
**②Look Modification Transform (LMT)**：这一步骤不是必需的，主要是给艺术家更方便地调色；  

③ 和 ④ 又合称为 Output Transform：  
**③Reference Rendering Transform (RRT)**：这步就是核心的一步，

Color transformation from scene-referred to display-referred space, using an S-shaped tone curve, image preparation for output

**④Output Device Transform (ODT)** ransforms to an output device, a transformation such as Rec. 709, DCI-P3, Rec.2020
