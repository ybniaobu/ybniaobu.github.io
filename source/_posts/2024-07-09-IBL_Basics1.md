...
> 好了，看到这我已经吓死了，看不懂，根本看不懂。但是不要慌，看不懂没关系，继续往下看！
...
就跟**傅里叶级数 Fourier series** 逻辑类似，其能将任意周期函数都表示成三角函数的线性组合。傅里叶级数的基就是 $\,sin(nx)\,$ 和 $\,cos(nx)\,$，这些基函数都是**正交的 orthogonal**。球谐基函数与傅里叶级数类似，是一组定义在球面坐标系上的基函数，对于任何 $\,f(r, \theta, \phi)\,$ 都可以表示为一组球谐基的和。既然可以使用球面坐标系，那也可以转换为直角坐标系，如下表：

*<center>表：直角坐标系下 3 阶球谐函数的系数</center>*
