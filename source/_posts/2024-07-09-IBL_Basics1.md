...
public class MonteCarloByComputeShader : OdinEditorWindow
{
    ...
    private void MonteCarloIntegrationCompute()
    {
        ...
        {
            float theta = Mathf.Asin(Random.value);
...
### 球谐函数简单理解
球谐函数和上面的两种方法（黎曼和和蒙特卡洛积分）不同，其本质就是使用球谐基函数来近似模拟光照函数，即任意球面函数可以由一组球谐基函数 $\,B_i(x)\,$ 和系数 $\,c_i\,$ 来表示：  

$$ f(x) = \sum_i^n c_i \times B_i(x) $$
...
因为基函数 $\,B_i(x)\,$ 有无数多个，我们只要取足够多个基函数，找到可以拟合 $\,f(x)\,$ 函数的每个基函数的系数 $\,c_i\,$，这样我们就可以用系数和基函数的组合来替换 $\,f(x)\,$。
