...
## Lighting Settings
我们可以使用 <kbd>Ctrl</kbd> + <kbd>9</kbd> 打开 Lighting Settings，这个 Lighting Settings 是 per scene 的，我们每个场景都可以配置。我们目前先只打开 Mixed Lighting 的 Baked Global Illumination，Lighting Mode 先选择为 Baked Indirect，Directional Mode 先设置为 Non-Directional：  
...
我同时也对比了只有方向光的情况下（Skybox 的 Intensity Multiplier 设置为 0），3 种不同 mode 下的结果，即 Realtime、Mixed、Baked。这里特别说明一下，Unity 的 LightMap 系统因为比较早，和现在的 PBR 模型在亮度上存在一个 PI 的差别，这也是为什么 Unity 在 URP 的直接光的 Diffuse 计算中没有除以 PI，以及 Specular 项多乘了 PI 的原因。所以在烘焙的时候我把直接光亮度设为了 1，而 Realtime Mode 下亮度设置为了 3.14：  
