...
还有一点，就是我们使用的 Cubemap 的压缩格式是 dLDR 和 RGBM 格式，就不能直接对 Cubemap 进行采样，否则颜色会变，此时就需要对 Cubemap 进行解码。首先需要在 `UnityPerDraw` CBuffer 中添加 `unity_SpecCube0_HDR`：  
...
## 自定义 Reflection Probe
Unity 自带的 Reflection Probe 的预过滤效果很差，特别是在有较亮光源的情况下。所以我打算自己弄个 Reflection Probe 工具，在某一位置上渲染到 Cubemap 再过滤，然后用 Unity 的 Reflection Probe 的 Custom 功能来使用自己预过滤的反射探针。
